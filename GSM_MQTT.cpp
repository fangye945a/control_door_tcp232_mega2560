...
String MQTT_HOST = "106.14.26.130";           //MQTT服务器IP      "192.168.10.108"
String MQTT_PORT = "1995";                    //MQTT服务器端口号

char tmp[128]={0};         //消息缓存数组
char csq[4]={"30"};          //信号强度满信号
  ...
  if(pingFlag == true)
  {
    ...
    if (_PingPrevMillis == 0 || (currentMillis - _PingPrevMillis ) >= _KeepAliveTimeOut * 1000)//如果时间大于心跳周期则发送ping包
    {
      ...
      if(_ping_return_flag > 1)				//如果发送2次心跳后还没有收到心跳回复则认为设备已离线
      {
			...
			digitalWrite(led_blue,LOW);      //蓝色led灯熄灭 
...
//#ifdef DE_BUG  
//      mySerial.print("ReceivedMessageType:");
//      mySerial.println(ReceivedMessageType,DEC);
//#endif	  
